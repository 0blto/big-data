...
* **Развертывание Superset**: добавление сервиса в docker-compose, настройка подключений к PostgreSQL
* **Создание витрин данных (Data Marts)** на основе fact и dim таблиц DDS:
...
* **Реализация ETL процессов для витрин**: DAG `dm_hh_etl`
* **Построение дашборда в Superset:**
* **Связь ETL витрин с DAG детального слоя:**
...
![Зарплаты в компаниях](readme_imgs/companies_salary.png)
![Зарплаты по дням](readme_imgs/min_max_salary_date.PNG)
![Зарплаты по вакансиям](readme_imgs/salary_from_to_name.PNG)
---

### Выводы

* Как и ожидалось самые высокие зарплаты в Москве, и верхняя и нижняя планки выше, чем в остальных городах, помимо этого в ней самая большая разница между минимальной и максимальной зарплатами.
* В конце предыдущего года практически не появлялись предложения работы, во время новогодних праздников их было так же крайне мало, по окончанию праздников произошел резкий скачок, сейчас требуется большое количество кадров.
* Если не учитывать вакансии, которые не удалось распределить по паттернам и они попали в "other / miscellanious", самая востребованная специальность в it - программист.
* Однако в среднем наибольший нижний и верхний пороги зарплат у менеджеров и лидов.
* Сложно отследить динамику предлагаемых зарплат по времени из-за колебаний и пропусков.
* Топ 3 компании с самыми высокими зарплатами:
  * САНЛАЙФ: 250-850 тысяч рублей
  * xHuman Sales: 450-700 тысяч рублей
  * Sminex
