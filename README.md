...
- ods_hh_vacancies
- ods_avito_vacancies)
Для ЛР2 используютсяя все эти 3 таблицы. На их основе строятся следующие dds таблицы:
...
В схеме «Звезда» факт-таблица напрямую связана с измерениями, без промежуточных таблиц. Это упрощает SQL-запросы и повышает производительность при агрегации.
Данные о рейсах имеют чёткую структуру «событие -> атрибуты»: каждый факт — это конкретная найденная вакансия, а атрибуты — зарплата, должность, компания... Это хорошо ложится на данный тип модели.
